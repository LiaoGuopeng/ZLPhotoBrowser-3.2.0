...
+ (void)requestSelectedImageForAsset:(ZLPhotoModel *)model isOriginal:(BOOL)isOriginal allowSelectGif:(BOOL)allowSelectGif completion:(nonnull void (^)(UIImage * _Nonnull, NSData * _Nullable, NSDictionary * _Nonnull))completion
{
    if (model.type == ZLAssetMediaTypeGif && allowSelectGif) {
        [self requestOriginalImageDataForAsset:model.asset progressHandler:nil completion:^(NSData *data, NSDictionary *info) {
            if (![[info objectForKey:PHImageResultIsDegradedKey] boolValue]) {
                ...
                if (completion) {
                    completion(image,data , info);
                }
            }
        }];
    } else {
        if (isOriginal) {
            [self requestOriginalImageForAsset:model.asset progressHandler:nil completion:^(UIImage * _Nonnull image, NSDictionary * _Nonnull info) {
                if (completion) {
                    completion(image,nil,info);
                }
            }];
        } else {
            CGFloat scale = 2;
            ...
            [self requestImageForAsset:model.asset size:size progressHandler:nil completion:^(UIImage * _Nonnull image, NSDictionary * _Nonnull info) {
                if (completion) {
                    completion(image,nil,info);
                }
            }];
