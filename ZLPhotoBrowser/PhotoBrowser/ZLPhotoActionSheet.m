...
- (void)requestSelPhotos:(UIViewController *)vc data:(NSArray<ZLPhotoModel *> *)data hideAfterCallBack:(BOOL)hide
{
    ...
    __block NSMutableArray *models = [NSMutableArray arrayWithCapacity:data.count];
    ...
    for (int i = 0; i < data.count; i++) {
        ...
        [ZLPhotoManager requestSelectedImageForAsset:model isOriginal:self.isSelectOriginalPhoto allowSelectGif:self.configuration.allowSelectGif completion:^(UIImage * _Nonnull image, NSData * _Nullable imageData, NSDictionary * _Nonnull info) {
            ...
            if (image) {
                LGPPhotoSelectModel *selectModel = [LGPPhotoSelectModel new];
                selectModel.type = model.type;
                selectModel.asset = model.asset;
                selectModel.image = image;
                selectModel.data = imageData;
                [models replaceObjectAtIndex:i withObject:selectModel];
            } else {
            ...
            if (self.selectModelBlock) {
                self.selectModelBlock(models, self.isSelectOriginalPhoto);
            }else{
                if (self.selectImageBlock) {
                    self.selectImageBlock([models valueForKeyPath:@"image"], [models valueForKeyPath:@"asset"], self.isSelectOriginalPhoto);
                    [self.arrSelectedModels removeAllObjects];
                }
            }
